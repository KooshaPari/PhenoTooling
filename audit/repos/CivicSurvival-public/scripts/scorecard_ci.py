...
PILLARS = [
    ...
    {"id": 10, "name": "DOCKERFILE", "check": lambda p: (p / "Dockerfile").exists()},
    {
        ...
        "name": "DOCKER_COMPOSE",
        "check": lambda p: (
            (p / "docker-compose.yml").exists() or (p / "docker-compose.yaml").exists()
        ),
    },
    ...
    },
    {
        ...
        "name": "PR_TEMPLATE",
        "check": lambda p: (
            any(p.glob(".github/PULL_REQUEST_TEMPLATE*"))
    ...
    },
    {
        ...
        "check": lambda p: (p / ".snyk").exists() or (p / "audit-ci.json").exists(),
    },
    {
        "id": 35,
        "name": "OPENAPI_SPEC",
    ...
    },
    {
        "id": 42,
        "name": "LOGGING",
        "check": lambda p: any(p.glob("**/logging.py")) or any(p.glob("**/*logger*")),
    },
    {
        "id": 43,
        "name": "MONITORING",
    ...
    },
    {
        "id": 54,
        "name": "ENV_VARS",
        "check": lambda p: (p / ".env").exists() or (p / ".env.local").exists(),
    },
    {
        "id": 55,
        "name": "KUBERNETES",
...
def audit_repo(repo_path):
    ...
    return {
        "score": score,
        "total": len(PILLARS),
        "percentage": (score / len(PILLARS)) * 100,
        "results": results,
    }
...
def load_baseline(path, total, expected_source_revision=None):
    ...
    if any(
        ...
        raise ValueError(
            f"Baseline passed_pillar_ids must contain unique pillar IDs from 1 through {total}"
        )
...
def main():
    ...
    try:
        ...
        report["score_delta"] = (
            report["score"] - baseline_score if baseline_score is not None else None
        )
        ...
        if args.fail_on_drop and (not report["target_met"] or report["regression"]):
            sys.exit(1)
    except Exception as e:
