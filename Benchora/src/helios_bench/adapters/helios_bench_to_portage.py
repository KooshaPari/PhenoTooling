...
if sys.argv[1:] in (["-h"], ["--help"]):
    program = sys.argv[0].replace("\\", "/").rsplit("/", 1)[-1]
    print(
        f"""usage: {program} [-h] --helios-bench-root HELIOS_BENCH_ROOT
                                  --portage-datasets PORTAGE_DATASETS
                                  [--limit LIMIT] [--dry-run]
...
options:
  ...
  --helios-bench-root HELIOS_BENCH_ROOT
                        Path to heliosBench checkout (must contain
                        src/helios_bench/tasks.py)
  --portage-datasets PORTAGE_DATASETS
                        Output directory for portage task trees
  --limit LIMIT         If set, only convert the first N tasks
  --dry-run             Build the task list but do not write to disk
  --summary-json SUMMARY_JSON
                        Optional path to write a JSON summary of converted
                        tasks"""
    )
    raise SystemExit(0)
