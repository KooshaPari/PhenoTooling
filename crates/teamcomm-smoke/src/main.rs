...
//!
//! Some `failed += 1` lines are immediately followed by `return`; this
//! is intentional so the final summary can still see the running counter.
//! Allow the unused-assignment lint at crate scope.
...
async fn main() -> anyhow::Result<()> {
    ...
    check!(
        "session.heartbeat",
        handlers::handle_session_heartbeat(state.clone(), json!({ "session_id": sid })).await
    );
    check!(
        "session.get",
        handlers::handle_session_get(state.clone(), json!({ "session_id": sid })).await
    );
    ...
    let _ = check!(
        "session.register (2nd)",
        handlers::handle_session_register(state.clone(), payload2).await
    );

    let list_resp = check!(
        "session.list",
        handlers::handle_session_list(state.clone(), json!({})).await
    );
    ...
    let claim = check!(
        "reservation.claim",
        handlers::handle_reservation_claim(
            state.clone(),
            json!({
    ...
    check!(
        "reservation.list",
        handlers::handle_reservation_list(state.clone(), json!({ "session_id": sid })).await
    );
    check!(
        "reservation.release",
        handlers::handle_reservation_release(
            state.clone(),
            json!({
    ...
    check!(
        "inbox.list",
        handlers::handle_inbox_list(state.clone(), json!({ "session_id": sid })).await
    );
    check!(
        "inbox.read",
        handlers::handle_inbox_read(
            state.clone(),
            json!({ "session_id": sid, "message_id": mid })
        )
        .await
    );
    ...
    check!(
        "state.get",
        handlers::handle_state_get(state.clone(), json!({ "session_id": sid })).await
    );
    ...
    check!(
        "discover.agents",
        handlers::handle_discover_agents(
            state.clone(),
            json!({
    ...
    check!(
        "session.deregister",
        handlers::handle_session_deregister(state.clone(), json!({ "session_id": sid })).await
    );
