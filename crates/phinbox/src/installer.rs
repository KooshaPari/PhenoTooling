//! `phinbox install` — copy the `phinbox` + `phinbox-mcp` binaries into
...
//!
//! ## macOS
//! - Writes `~/.local/bin/phinbox` + `~/.local/bin/phinbox-mcp`
//! - Appends a `path_prepend ~/.local/bin` line to `~/.zshrc` + `~/.bashrc`
//!   (only if not already present)
//! - Adds LaunchAgent plist `~/Library/LaunchAgents/com.phenotype.phinbox.plist`
//!   pointing at `phinbox daemon` for inbox-style tray integration
//!
//! ## Windows
//! - Writes `%LOCALAPPDATA%\phinbox\bin\phinbox.exe` and `phinbox-mcp.exe`
//! - Adds that dir to the user PATH via `setx PATH "%PATH%;%LOCALAPPDATA%\phinbox\bin"`
//! - Schedules a startup task via `schtasks /create /tn PhinboxDaemon /tr ...`
//!
//! ## Linux
//! - Writes `~/.local/bin/phinbox` + `~/.local/bin/phinbox-mcp`
//! - Appends to `~/.bashrc`
//! - Installs systemd user unit at `~/.config/systemd/user/phinbox.service`
...
pub struct InstallOptions {
    ...
    /// run `phinbox install --with-shell-rc`.
...
/// (`PHINBOX_SRC_BIN`) and fall back to `current_exe()`.
