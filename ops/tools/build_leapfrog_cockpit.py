...
Inputs:
    - beads.jsonl (repository-root authoritative source of truth)
...
BEADS_PATH = REPO_ROOT / "beads.jsonl"
...
# first.  These were hand-curated by inspecting the Aug 10 bead ledger.
...
# ---------------------------------------------------------------------------
def load_beads(
    sources: tuple[Path, ...] = BEAD_SOURCES,
    snapshots: dict[Path, bytes] | None = None,
) -> list[dict[str, Any]]:
    beads: list[dict[str, Any]] = []
    seen: set[tuple[str, str, str, str]] = set()
    for path in sources:
        content = snapshots.get(path) if snapshots is not None else None
        if content is None:
            if not path.exists():
...
def render_card(b: dict[str, Any], lane_id: str) -> str:
    ...
    return (
        ...
        f'data-c="{cmd_attr}" data-t="{target}" '
...
def input_provenance(
    path: Path, content: bytes, *, is_live_root: bool
) -> dict[str, str | int | bool]:
    """Return displayable integrity facts for the exact selected JSONL input."""
    return {
        "name": path.name,
        "sha256": hashlib.sha256(content).hexdigest(),
        "records": sum(1 for line in content.splitlines() if line.strip()),
        "is_live_root": is_live_root,
    }


def render_footer(provenance: dict[str, str | int | bool]) -> str:
    source_name = str(provenance["name"])
    if provenance["is_live_root"]:
        source_statement = """
      <p>beads.jsonl (repository root) is authoritative; this HTML is a derived view.
      It is rendered from that append-only ledger only.</p>
      <p class=\"provenance-warning\">Input class: live root ledger. Historical continuity is incomplete;
      this view must not be read as a recovery of the overwritten artifact.</p>"""
    else:
        source_statement = """
      <p>Input class: historical custody ledger. This HTML is a derived projection only,
      not historical artifact recovery.</p>"""
    return f"""
    <footer class="footer">
      <h2>Source of Truth</h2>
      {source_statement}
      <p>Selected ledger: <code>{esc(str(provenance["name"]))}</code>; SHA-256
      <code>{esc(str(provenance["sha256"]))}</code>; {provenance["records"]} records.</p>
      ...
      Generated from <code>{esc(source_name)}</code>.
...
function onBeadClick(el) {
  var cmd = el.getAttribute('data-c') || '';
...
def preserved_appendix(path: Path, source_name: str) -> str:
    """Retain existing post-document records while normalizing retired source text."""
    existing = path.read_text(encoding="utf-8")
    _, separator, appendix = existing.partition("</html>")
    if not separator:
        return ""
    return (
        appendix.replace("~/.agileplus/audit.jsonl", source_name)
        .replace("phenotype-dag/beads.jsonl", source_name)
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the approved cockpit from one JSONL ledger.")
    parser.add_argument(
        "--ledger",
        type=Path,
        help="explicit preserved JSONL custody ledger; renders a projection, never artifact recovery",
    )
    # Imported callers (including tests and automation) must not inherit the
    # host process's arguments. The executable entry point passes sys.argv[1:].
    args = parser.parse_args([] if argv is None else argv)
    if not OUTPUT_PATH.exists():
        print(f"ERROR: approved target does not exist: {OUTPUT_PATH}", file=sys.stderr)
        return 1

    ledger_path = args.ledger if args.ledger is not None else BEADS_PATH
    if not ledger_path.is_file():
        print(f"ERROR: selected ledger does not exist: {ledger_path}", file=sys.stderr)
        return 1
    snapshot = ledger_path.read_bytes()
    is_live_root = ledger_path.resolve() == BEADS_PATH.resolve()
    provenance = input_provenance(ledger_path, snapshot, is_live_root=is_live_root)
    beads = load_beads((ledger_path,), snapshots={ledger_path: snapshot})
    if not beads:
        print("ERROR: no beads loaded", file=sys.stderr)
        return 1
    print(f"loaded {len(beads)} beads from {ledger_path}", file=sys.stderr)
    ...
    footer_html = render_footer(provenance)
...
<header class="banner">
  ...
  <div class="subtitle">Build 2026-08-10 \u2014 9-lane AgilePlus kanban derived from {esc(str(provenance["name"]))}</div>
...
"""
    ...
    doc += preserved_appendix(OUTPUT_PATH, str(provenance["name"]))

    # The ledger has grown past the point where formatting whitespace alone can
    # keep the derived, self-contained view below its declared byte cap. Remove
    # only whitespace between HTML tags: card text, attributes, and preserved
    # appendix content remain byte-for-byte intact.
    doc = re.sub(r">\s+<", "><", doc)
